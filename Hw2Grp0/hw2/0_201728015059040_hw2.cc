...
public:
    void compute(MessageIterator* pmsgs) {
      ...
      // iteratelly calculate the distance between vertexs
      if (getSuperstep() >= 2) {
      ...
      // if the val of current vertex changed
      // or is in the 0th superstep
      // then modify the val of current vertex
      if(val < getValue() || getSuperstep() == 0) {
          *mutableValue() = val;

          OutEdgeIterator otherEdge = getOutEdgeIterator();
          while (!otherEdge.done()) {
      ...
      // halt this vertex, wait for next message
      voteToHalt();
    }
};
